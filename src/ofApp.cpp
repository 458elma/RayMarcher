...
// signed distance function for the whole scene
float ofApp::sceneSDF(const glm::vec3 point, int &index) {
	float closestDist = INFINITY;
	for (int i = 0; i < scene.size(); i++) {
		float dist = scene[i]->sdf(point);
		if (dist < closestDist) {
			closestDist = dist;
			index = i;
		}
...
// get normal of a point for ray marching
glm::vec3 ofApp::getNormalRM(const glm::vec3 &p) {
	int inx = 0;
	float eps = 0.01;
	float dp = sceneSDF(p, inx);
	glm::vec3 n(dp - sceneSDF(glm::vec3(p.x - eps, p.y, p.z), inx), 
		dp - sceneSDF(glm::vec3(p.x, p.y - eps, p.z), inx), 
		dp - sceneSDF(glm::vec3(p.x, p.y, p.z - eps), inx));
	return glm::normalize(n);
}

// ray marching hit detection algorithm
bool ofApp::rayMarch(Ray r, glm::vec3 &p, int &index) {
	bool hit = false;

	p = r.p;
	for (int i = 0; i < MAX_RAY_STEPS; i++) {
		float dist = sceneSDF(p, index);
...
void ofApp::rayMarchLoop() {
	...
	for (int y = 0; y < imageHeight; y++) {
		for (int x = 0; x < imageWidth; x++) {
				...
				bool hit = rayMarch(toShoot, hitPt, ind); // used to indicate if any intersection happened
...
// lambert shading algorithm for ray marching
ofColor ofApp::lambertRM(const glm::vec3 &p, const glm::vec3 &norm, const ofColor diffuse, SceneObject* s) {
	float ambientInt = 0.07;

	ofColor ld = 0;
	// loop through all lights to apply proper diffuse shading
	for (Light* light : lights) {
		Ray shadowRay(p + (0.0101 * norm), glm::normalize(light->position - p));
		glm::vec3 pt;
		int indx = 0;
		bool shadHit = false;
		// ray march with shadow ray to see if point is blocked from light source
		// no shadow casted on meshes and height fields
		Mesh* mPtr = dynamic_cast<Mesh*>(s);
		HeightField* hfPtr = dynamic_cast<HeightField*>(s);
		Repeater* rPtr = dynamic_cast<Repeater*>(s);
		if (mPtr == nullptr && hfPtr == nullptr && rPtr == nullptr) {
			shadHit = rayMarch(shadowRay, pt, indx);
		}
...
// phong shading algorithm for ray marching
ofColor ofApp::phongRM(const glm::vec3 &p, const glm::vec3 &norm, const ofColor diffuse, const ofColor specular, float power, SceneObject* s) {
	...
	ls = ls + lambertRM(p, norm, diffuse, s); // add the lambert shading done in the lambert function to get full shading
...
void ofApp::setup(){
	...
	float moveZ = -7;
	//scene.push_back(new Sphere(glm::vec3(0, 0, -2 + moveZ), 1.0, ofColor::red));
	//scene.push_back(new Sphere(glm::vec3(2.5, 0, -2 + moveZ), 0.75, ofColor::red));
	//scene.push_back(new Sphere(glm::vec3(-2.5, 0, -2 + moveZ), 0.75, ofColor::red));
	//scene.push_back(new Torus(glm::vec3(4, 3, -4 + moveZ), glm::vec3(90, 45, 45), glm::vec3(1, 1, 1), 1.0, glm::vec2(1, 0.25), ofColor::green));
	//scene.push_back(new Torus(glm::vec3(0, 3, -4 + moveZ), glm::vec3(90, 0, 0), glm::vec3(1.5, 1.5, 1.5), 1.0, glm::vec2(1, 0.25), ofColor::green));
	//scene.push_back(new Torus(glm::vec3(-4, 3, -4 + moveZ), glm::vec3(90, -45, -45), glm::vec3(1, 1, 1), 1.0, glm::vec2(1, 0.25), ofColor::green));
	//scene.push_back(new Cylinder(glm::vec3(-6, -1, -4 + moveZ), glm::vec3(45, 45, 45), glm::vec3(1, 1, 1), 0.5, 1.5, ofColor::blue));
	//scene.push_back(new Cylinder(glm::vec3(0, -1, -4 + moveZ), glm::vec3(0, 0, 90), glm::vec3(1.5, 1.5, 1.5), 0.5, 1.5, ofColor::blue));
	//scene.push_back(new Cylinder(glm::vec3(6, -1, -4 + moveZ), glm::vec3(45, -45, -45), glm::vec3(1, 1, 1), 0.5, 1.5, ofColor::blue));
	...
	heightField = new HeightField(glm::vec3(0, -2, 0 + moveZ), 2, 1, 1, ofColor::purple);
	//scene.push_back(heightField);
	//scene.push_back(new Torus(glm::vec3(0, 0, 0), glm::vec3(0, 0, 0), glm::vec3(1, 1, 1), 1.0, glm::vec2(1, 0.25), ofColor::green));
	//scene.push_back(new Repeater(glm::vec3(0, 0, 0), glm::vec2(1, 0.25), ofColor::gold));

	/* Fractals for CS116B Final Project */
	scene.push_back(new Sponge(glm::vec3(-2, -1, 2.5)));
	scene.push_back(new Sierpinski(glm::vec3(1, 1.5, 0)));
	scene.push_back(new SierDodeca(glm::vec3(3.5, -2.5, -3)));
	//scene.push_back(new Sponge(glm::vec3(0, 0, -8), glm::vec3(0, 0, 0), glm::vec3(10, 10, 10), ofColor::gold, 5));
	//scene.push_back(new Sierpinski(glm::vec3(0, 0, 0), glm::vec3(-45, 0, -45), glm::vec3(5, 5, 5), ofColor::pink, 5));
	//scene.push_back(new SierDodeca(glm::vec3(0, 0.75, 4), glm::vec3(45, 0, 0), glm::vec3(4, 4, 4), ofColor::turquoise, 6));
	...
	// setup sliders
	gui.setup();
	gui.add(lightIntensity.setup("Top Light Intensity", 150, 0, 500));
	gui.add(lightIntensity2.setup("Left Light Intensity", 150, 0, 500));
	gui.add(lightIntensity3.setup("Back Light Intensity", 150, 0, 500));
	gui.add(powerExp.setup("Phong Exponent", 109, 10, 10000));
	gui.add(frequent.setup("Frequency", 0, 0, 4));
	gui.add(amplit.setup("Amplitude", 1, 0, 5));
	gui.add(octv.setup("Octave", 1, 1, 20));
	gui.add(gridsAA.setup("Anti-aliasing dimension", 5, 2, 10));

	// setup lights
	lights.push_back(new Light(glm::vec3(13, 8, 8 + moveZ), lightIntensity));
	lights.push_back(new Light(glm::vec3(-13, 5, 13 + moveZ), lightIntensity2));
	lights.push_back(new Light(glm::vec3(0, 0, -15 + moveZ), lightIntensity3));
	//lights.push_back(new Light(glm::vec3(0, 0, 20 + moveZ), 50));
	//lights.push_back(new Light(glm::vec3(0, 0, 4), lightIntensity));
...
void ofApp::update(){
	...
	//lights[3]->changeIntensity(lightIntensity3);
	//lights[3]->changeIntensity(lightIntensity);
	// update number of floor tiles to use
	//floorPlane->setNumTiles(tiles);
	heightField->frequency = frequent;
	heightField->amplitude = amplit;
	heightField->octave = octv;
}
...
void ofApp::keyPressed(int key){
	...
	case 'r':
		rayTrace();
		break;
	// press m to render with ray marching
	case 'M':
	case 'm':
		rayMarchLoop();
		break;
